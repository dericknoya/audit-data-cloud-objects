...
async def audit_dmo_fields():
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        ...
        group_by_tasks = [
            fetch_api_data(session, instance_url, f"/services/data/v64.0/query?{urlencode({'q': f'SELECT Activation_Id__c FROM {dmo.get("name")} GROUP BY Activation_Id__c'})}", semaphore, 'records')
        ...
        for i, records in enumerate(results_by_dmo):
            if records:
                ...
                activation_ids_by_dmo[dmo_name] = [rec['Activation_Id__c'] for rec in records if rec.get('Activation_Id__c')]
