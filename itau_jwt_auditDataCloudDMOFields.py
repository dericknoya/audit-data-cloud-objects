...
async def fetch_paginated_data_with_offset(session, instance_url, relative_url, semaphore, key_name):
    """Função especializada para endpoints que usam paginação por offset."""
    async with semaphore:
        all_records = []
        offset = 0
        batch_size = 200 # Um tamanho de lote razoável para evitar muitas chamadas
        
        logging.info(f"Iniciando busca com paginação por offset em: {relative_url}")
        while True:
