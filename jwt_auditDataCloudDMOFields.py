...
async def main():
    ...
    if not data_kits:
        logging.warning("⚠️ Não foi possível buscar os Data Kits devido a um erro de API. A análise de dependências continuará, mas Data Streams utilizados exclusivamente por Data Kits podem ser incorretamente classificados como órfãos.")
        ...
        if not is_published_recently:
            ...
            if not is_used_as_filter:
                ...
                if not deletion_identifier:
                    logging.warning(f"Não foi possível encontrar o 'apiName' para o segmento {get_segment_name(seg)} (ID: {seg_id}). Este segmento não poderá ser deletado pelo script.")
                ...
                for act in activations:
                    if str(act.get('segmentId') or '')[:15] == seg_id:
                        ...
                        audit_results.append({
                            'DELETAR': 'NAO', 'ID_OR_API_NAME': act_id, 'DISPLAY_NAME': act_name, 'OBJECT_TYPE': 'ACTIVATION',
                            'REASON': act_reason, 'TIPO_ATIVIDADE': 'N/A', 'DIAS_ATIVIDADE': 'N/A', 'DELETION_IDENTIFIER': act_id
                        })
            else:
    ...
    for dmo in dm_objects:
        ...
        is_unused = (normalized_dmo_name not in dmos_used_by_segments and 
                     ...
                     normalized_dmo_name not in dmos_used_by_data_actions)
        if is_unused:
