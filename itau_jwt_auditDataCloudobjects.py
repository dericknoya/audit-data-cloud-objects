...
async def main():
    ...
    async with aiohttp.ClientSession(headers=headers, base_url=instance_url, connector=aiohttp.TCPConnector(ssl=VERIFY_SSL)) as session:
        ...
        for dmo_record in dmo_tooling_data:
            dmo_name = dmo_record.get('DeveloperName')
            if not dmo_name or not dmo_name.endswith('__dlm') or any(dmo_name.lower().startswith(p) for p in dmo_prefixes_to_exclude):
                continue
            ...
            if normalized_dmo_name not in all_used_dmos:
                ...
                if not created_date or created_date < ninety_days_ago:
                    ...
                    display_name = get_dmo_display_name(dmo_record)
                    creator_id = dmo_record.get('CreatedById')
                    creator_name = user_id_to_name_map.get(creator_id, 'Desconhecido')
                    
                    if creator_name == 'Desconhecido' and creator_id:
                        logging.info(f"DEBUG: Não foi possível encontrar o nome para o criador do DMO '{dmo_name}' com ID: {creator_id}")
                    
                    deletion_id = dmo_name
                    
                    audit_results.append({
                        'DELETAR': 'NAO', 
                        'ID_OR_API_NAME': dmo_record.get('Id', 'ID não encontrado'), 
