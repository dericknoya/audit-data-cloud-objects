...
Versão: 22.01 (Debug de Mapeamentos)
- BASE: v22.00
- RECURSO DE DEBUG: Adicionada instrumentação para diagnosticar o problema de
  mapeamentos não encontrados. O script irá gerar:
  - 'debug_mapping_calls.csv': Registra cada URL consultada na API de mapeamentos.
  - 'debug_mapping_responses.json': Salva a resposta bruta da API para cada
    consulta de mapeamento, permitindo a análise da estrutura do JSON.
- LOG ADICIONAL: Incluído um log para exibir uma amostra dos DMOs que estão
  sendo enviados para a verificação de mapeamento.
"""
import os
import time
import asyncio
...
async def main():
    ...
    logging.info("🚀 Iniciando auditoria de objetos v22.01 (Debug de Mapeamentos)...")
    ...
    async with SalesforceClient(config, auth_data) as client:
        ...
        logging.info(f"Amostra de DMOs a serem verificados: {dmos_to_check_for_mappings[:5]}")
